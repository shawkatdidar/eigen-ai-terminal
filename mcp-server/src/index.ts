...
interface RadarData {
  ...
  ripples: {
    significant: Record<number, RippleData>;
    notable: Record<number, RippleData>;
  };
}
...
const DATA_URL =
  process.env.EIGEN_DATA_URL || "https://terminal.clawlab.dev/data";
const WIKI_URL =
  process.env.EIGEN_WIKI_URL || "https://terminal.clawlab.dev/wiki";

function readLocalFile(...candidates: string[]): string | null {
...
function wikiFileCandidates(filePath: string): string[] {
  return [
...
async function loadHistory(): Promise<HistoryEntry[]> {
  ...
  try {
    ...
    if (res.ok) {
      cachedHistory = (await res.json()) as HistoryEntry[];
      historyCacheTime = now;
      return cachedHistory!;
    }
  } catch {
...
function fmtSignal(s: Signal, significance: string): string {
  const domains =
    s.nodeNames?.length > 0
      ? s.nodeNames.join(", ")
      : s.nodes.map(dn).join(", ");
...
function fmtRipple(r: RippleData): string {
  const parts: string[] = [];
  if (r.chains?.length) {
    parts.push("**What this causes:**");
    for (const c of r.chains)
      parts.push(
        `- ${c.title} (${c.direction}, ${c.strength}): ${c.mechanism}`
      );
  }
  if (r.convergences?.length) {
    ...
    for (const c of r.convergences)
      parts.push(
        `- ${c.title} (${c.confidence} confidence): ${c.predictedOutcome}`
      );
  }
  ...
  if (r.predictions?.length) {
    ...
    for (const p of r.predictions)
      parts.push(
        `- ${p.title} (${p.confidence}, check by ${p.checkDate})`
      );
  }
  return parts.join("\n");
}
...
server.tool(
  ...
  "Get today's AI intelligence — all signals from the latest scan with significance levels, domains, actionability flag, and tags. The agent should first determine what the user is building, then use that context to decide what to surface. For the morning brief, pick 2-3 actionable items. If any item needs more detail or verification, call about() or read() instead of filling gaps from model memory. Returns ripple effects for significant signals.",
  {
    significance: z
      ...
      .default("all")
      .describe(
        "Filter by significance. 'significant' = major developments. 'notable' = worth knowing. 'breakthrough' = rare, industry-changing."
      ),
    only_actionable: z
      .boolean()
      .default(false)
  ...
  async ({ significance, only_actionable }) => {
    ...
    type Tagged = { signal: Signal; sig: string; idx: number };
    const all: Tagged[] = [];

    if (significance === "all" || significance === "breakthrough")
      (data.brief.breakthrough || []).forEach((s, i) =>
        all.push({ signal: s, sig: "breakthrough", idx: i })
      );
    if (significance === "all" || significance === "significant")
      data.brief.significant.forEach((s, i) =>
        all.push({ signal: s, sig: "significant", idx: i })
      );
    if (significance === "all" || significance === "notable")
      data.brief.notable.forEach((s, i) =>
        all.push({ signal: s, sig: "notable", idx: i })
      );
    ...
    lines.push(
      'Use about("[topic]") to go deeper on any signal. Use changes() to check for updates later.'
    );
...
server.tool(
  "about",
  "Get everything we know about a topic, company, model, technology, or AI domain — all in one call. Returns: wiki content, recent signals, related trends, blockers, and predictions. Use when the user asks about something specific. Examples: about('Anthropic'), about('MCP'), about('AI coding tools'), about('open source models').",
  {
    topic: z
      .string()
      .describe(
        "What to look up — a company, model, technology, concept, or AI domain"
      ),
  },
  async ({ topic }) => {
    const data = await loadData();
    ...
    const entityHit = data.entityIndex
      ? Object.entries(data.entityIndex).find(
          ([name]) =>
    ...
    if (entityHit) filesToTry.push(entityHit[1]);
    ...
    for (const f of manifest.files) {
      ...
      if (
        ...
        if (!filesToTry.includes(f.path)) filesToTry.push(f.path);
      }
    }
    ...
    const allSignals = [
      ...data.brief.significant.map((s, i) => ({
        ...s,
        sig: "significant" as const,
        idx: i,
      })),
      ...data.brief.notable.map((s, i) => ({
        ...s,
        sig: "notable" as const,
        idx: i,
      })),
    ];

    const hitSignals = allSignals.filter((s) =>
      matches(
        s.title +
          " " +
          s.description +
    ...
    // 3. Matching trends
    const hitTrends = data.convergences.filter((c) =>
      matches(
        c.title +
          " " +
          c.predictedOutcome +
          " " +
          c.forces.map((f) => f.description + " " + dn(f.origin)).join(" "),
        topic
      )
    );
    ...
    if (hitTrends.length > 0) {
      ...
      for (const t of hitTrends) {
        lines.push(`### ${t.title}`);
        lines.push(
          `Confidence: ${t.confidence} | Timeline: ${t.timeline || "uncertain"}`
        );
    ...
    // 4. Matching blockers
    const hitBlocked = data.bottlenecks.filter((b) =>
      matches(
        b.title + " " + b.summary + " " + b.blocks.map(dn).join(" "),
        topic
      )
    );
    ...
    if (hitBlocked.length > 0) {
      ...
      for (const b of hitBlocked) {
        lines.push(`### ${b.title}`);
        lines.push(
          `Status: ${b.status} | Blocks: ${b.blocks.map(dn).join(", ")}`
        );
        lines.push(b.summary);
        lines.push(
          `${b.attackers} teams working on this | ${b.looseningSignals} signs of progress`
        );
        lines.push("");
      }
    }
    ...
    const hitPreds = data.predictions.filter((p) =>
      matches(p.title + " " + (p.basedOn || ""), topic)
    );
    ...
    if (hitPreds.length > 0) {
      ...
      for (const p of hitPreds)
        lines.push(
          `- **${p.title}** — ${p.confidence} confidence, check by ${p.checkDate}`
        );
      lines.push("");
    }
    ...
    const relatedPages = manifest.files
      .filter((f) => {
        const name = ((f.frontmatter.name as string) || "").toLowerCase();
        return (
          f.tags.some((t) => t.toLowerCase().includes(topicLower)) ||
    ...
    if (relatedPages.length > 0) {
      ...
      for (const p of relatedPages) {
        const name = (p.frontmatter.name as string) || p.path;
    ...
    if (!foundContent) {
      lines.push(
        "No information found for this topic. Try a different name or broader term."
      );
      lines.push(
...
server.tool(
  "ripple",
  "Trace the cause-and-effect chain of a specific signal — what it pushes, what trends it feeds, what blocks it, and what we predict next. Pass the signal title or a keyword to match. Use when the user asks 'what does this mean?' or 'what happens because of this?'",
  {
    signal: z
      .string()
      .describe(
        "The signal title or a keyword to match against today's signals"
      ),
  },
  async ({ signal }) => {
    const data = await loadData();

    type Tagged = Signal & { sig: string; idx: number };
    const all: Tagged[] = [
      ...data.brief.significant.map(
        (s, i) => ({ ...s, sig: "significant", idx: i }) as Tagged
      ),
      ...data.brief.notable.map(
        (s, i) => ({ ...s, sig: "notable", idx: i }) as Tagged
      ),
    ];
    ...
    let matched =
      all.find((s) => s.title.toLowerCase().includes(queryLower)) ||
      all.sort(
        (a, b) =>
          matchStrength(b.title + " " + b.description, signal) -
          matchStrength(a.title + " " + a.description, signal)
      )[0];

    if (!matched || matchStrength(matched.title + " " + matched.description, signal) === 0) {
      return txt(
        `No signal matching "${signal}".\n\nToday's signals:\n${all.map((s) => `- ${s.title}`).join("\n")}`
      );
    }
    ...
    if (ripple) {
      ...
      const relTrends = data.convergences.filter((c) =>
        c.forces.some((f) => sigNodes.has(f.origin))
      );
      const relBlocked = data.bottlenecks.filter((b) =>
        b.blocks.some((x) => sigNodes.has(x))
      );
      const relChains = data.forceChains.filter(
        (fc) =>
          fc.status === "active" &&
      ...
      if (relChains.length > 0) {
        ...
        for (const c of relChains.slice(0, 3))
          lines.push(
            `- ${c.title} (${c.direction}): ${c.mechanism.slice(0, 250)}`
          );
        lines.push("");
      }
      if (relTrends.length > 0) {
        ...
        for (const t of relTrends)
          lines.push(
            `- ${t.title} (${t.confidence} confidence): ${t.predictedOutcome}`
          );
        lines.push("");
      }
    ...
    lines.push("");
    lines.push(
      `Use about("${matched.nodes[0] ? dn(matched.nodes[0]) : signal}") for broader context.`
    );
    return txt(lines.join("\n"));
  }
);
...
server.tool(
  "trends",
  "Get developing trends in AI — patterns where 3+ independent signals point at the same outcome. Each trend has contributing forces from different domains, a confidence level, a timeline, and what would disprove it. Use when the user asks 'what trends should I watch?' or wants forward-looking context.",
  {},
  async () => {
    const data = await loadData();
    ...
    const sorted = [...data.convergences].sort(
      (a, b) =>
        (confRank[b.confidence.toLowerCase()] ?? 1) -
...
server.tool(
  "blocked",
  "Get what's holding AI progress back — constraints affecting multiple areas at once. Shows what's blocked, who's working on it, and signs of progress. Use when the user asks about limitations, risks, or 'what's the catch?'",
  {},
  async () => {
    const data = await loadData();
    const lines = [
    ...
    for (const b of data.bottlenecks) {
      lines.push(`## ${b.title}`);
      lines.push(
        `Status: ${b.status} | Type: ${b.type || "constraint"}`
      );
      ...
      lines.push("");
      lines.push(
        `${b.attackers} teams/companies working on this | ${b.looseningSignals} signs of progress`
      );
      lines.push("", "---", "");
    }
...
server.tool(
  "speed",
  "Get rate-of-change metrics — how fast key things in AI are moving. Costs, capabilities, adoption, capital. Use when the user needs quantitative context, asks 'how fast is X changing?', or wants to understand momentum.",
  {
    category: z
      .enum(["all", "cost", "capability", "capital", "adoption"])
      .default("all")
      .describe(
        "Filter: cost (pricing), capability (model performance), capital (funding), adoption (usage growth)"
      ),
  },
  async ({ category }) => {
    const data = await loadData();
    ...
    const groups: Record<string, VelocityMetric[]> = {};
    for (const v of filtered) (groups[v.category || "Other"] ??= []).push(v);
    ...
    for (const [cat, metrics] of Object.entries(groups)) {
      ...
      for (const v of metrics) {
        ...
        lines.push(
          `  Rate of change: ${v.velocity} | Acceleration: ${v.acceleration}`
        );
        if (v.date) lines.push(`  Last measured: ${v.date}`);
      }
      lines.push("");
    }
...
server.tool(
  "predictions",
  "Get specific, dated predictions about where AI is heading — each with a confidence level and check date. We track whether they come true. Use when the user wants forward-looking context or asks 'what's coming next?'",
  {},
  async () => {
    const data = await loadData();
    ...
    for (const p of sorted) {
      ...
      if (p.basedOn) lines.push(`Based on: ${p.basedOn}`);
      lines.push("");
    }
...
server.tool(
  "search",
  "Search across the entire knowledge base — domain trackers, company profiles, daily briefs, frameworks. Returns matching files with context snippets. Use when looking for specific info or when about() didn't find enough.",
  {
    query: z
      .string()
      .describe(
        "What to search for — a company, technology, concept, model name, or keyword"
      ),
    max_results: z
      .number()
      .default(10)
    ...
    const scored = await Promise.all(
      manifest.files.map(async (f) => {
        ...
        if (score > 0) {
          ...
          if (raw) {
            if (raw.toLowerCase().includes(queryLower)) score += 5;
            for (const line of raw.split("\n")) {
              const ll = line.toLowerCase();
              if (
                ll.includes(queryLower) ||
...
server.tool(
  "read",
  "Read a specific page from the knowledge base. Returns full content with navigation links to related pages. Use after search() or about() points you to a file. Example paths: 'nodes/ai-agents.md', 'entities/anthropic.md', 'briefs/2026-04-08.md'.",
  {
    file_path: z
      .string()
...
server.tool(
  "changes",
  "Get what's new since a specific date. Returns new signals and updates. Use between morning briefs to check for breaking developments, or when the user hasn't checked in a while. If something significant is found, consider alerting the user proactively.",
  {
    since: z
      .string()
      .describe(
        "Date to check from, e.g. '2026-04-07' or '2026-04-07T07:00:00Z'"
      ),
  },
  async ({ since }) => {
    ...
    for (const entry of history) {
      ...
      if (d >= sinceDate && entry.date !== data.brief.date) {
        for (const s of entry.signals) items.push({ date: entry.date, ...s });
      }
    }

    if (items.length === 0) {
      return txt(
        `No new signals since ${fmtDate(since)}. Check back later or use today() for the latest full scan.`
      );
    }
    ...
    if (sigItems.length > 0) {
      ...
      for (const s of sigItems) {
        lines.push(`### ${s.title}`);
        lines.push(
          `Date: ${s.date} | Domains: ${s.domains.join(", ")} | Actionable: ${s.practical ? "Yes" : "No"}`
        );
    ...
    if (notItems.length > 0) {
      ...
      for (const s of notItems) {
        lines.push(
          `- **${s.title}** (${s.date}) — ${s.description.slice(0, 200)}`
        );
      }
      lines.push("");
    }
...
server.tool(
  ...
  async ({ days }) => {
    ...
    if (!raw) return txt("No updates available.");
...
server.tool(
  ...
  async ({ since }) => {
    ...
    try {
      ...
      lines.push(
        "If this is relevant to the user's work, tell them. If not, skip it."
      );
      lines.push(
...
async function main() {
  ...
  console.error(
    "Eigen AI Terminal MCP server v1.0.0 — 12 tools available"
  );
}
