...
interface RadarData {
  ...
  convergences: Array<{
    ...
    timeline: string;
    forces: Array<{ description: string; origin: string }>;
  }>;
  bottlenecks: Array<{
    id: string;
    title: string;
...
function relevanceScore(text: string, context: string): number {
  ...
  for (const word of textWords) {
    if (contextWords.has(word)) score++;
  }
  return score;
}
...
server.tool(
  "radar_brief",
  "Get today's AI radar signals. Returns significant and notable developments across the AI landscape. Use 'view' to filter: 'builder' for tools/models/tech, 'strategic' for funding/policy/market, 'all' for everything.",
  {
  ...
  async ({ view, significance }) => {
    ...
    let signals: Array<{ title: string; description: string; nodeNames: string[]; view: string }> = [];
...
server.tool(
  "radar_signal_ripple",
  "Get the causal ripple effects of a specific signal — what it's pushing, what trends it feeds, what it's blocked by, and what we predict. Pass the signal number (0-indexed from today's significant signals).",
  {
  ...
  async ({ signal_index }) => {
    ...
    if (!sig) {
      return { content: [{ type: "text" as const, text: `No signal at index ${signal_index}. Today has ${data.brief.significant.length} significant signals (0-${data.brief.significant.length - 1}).` }] };
    }
...
server.tool(
  "radar_trends",
  "Get developing trends in the AI landscape — when multiple independent signals point at the same outcome. Returns trends with confidence levels, timelines, and contributing forces.",
  {},
  async () => {
    const data = await loadData();
    const text = [
...
server.tool(
  "radar_roadblocks",
  "Get roadblocks holding back AI progress — constraints blocking multiple areas simultaneously. Shows what's blocked, who's attacking it, and signs of progress.",
  {},
  async () => {
    const data = await loadData();
    const text = [
...
server.tool(
  "radar_velocity",
  "Get velocity metrics — how fast key things are changing in AI. Shows current values, rate of change, and whether they're accelerating or decelerating.",
  {
...
server.tool(
  "radar_predictions",
  "Get AI Radar's predictions — specific, dated, falsifiable claims with confidence levels and check dates. We track whether each one comes true.",
  {},
  async () => {
    const data = await loadData();
    const text = [
...
server.tool(
  "radar_relevant",
  "Get radar intelligence relevant to a specific context. Describe what you're building or working on, and this returns only the signals, trends, roadblocks, and predictions that matter for that work. This is the primary tool for personalized intelligence.",
  {
